...
const MiPerfil = () => {
  ...
  const onSubmit = handleSubmit((data: any) => {
    ...
    try {
        const request = {id: usuario.id ,nombre: data.nombre, username: data.username, email: data.email,  rol:usuario.rol};
        console.log(request);
  ...
  useEffect(() => {
    const getUsuario = async () => {
      try {
        const response = await axios.get(USUARIOS_API_BASE_URL + "/me", {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer " + localStorage.getItem("token"),
    ...
    <>
      ...
      {usuario && (
        <form className="max-w-md mx-auto mt-10" onSubmit={onSubmit}>
          ...
          {
              errors.nombre && (
                  <span className="text-red-500 text-xs italic ms-20">{errors.nombre.message as string}</span>
              )
          }
          ...
          {
            errors.username && (
                <span className="text-red-500 text-xs italic ms-20">{errors.username.message as string}</span>
            )
          }
          ...
          {
            errors.email && (
                <span className="text-red-500 text-xs italic flex ms-20">{errors.email.message as string}</span>
            )
          }
