...
const FormCircuitos = () => {
    ...
    let trazado = '';
    let trazadoPath = '';
    const supabase = createClient("https://pxfvrkflonlookyusxtb.supabase.co", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InB4ZnZya2Zsb25sb29reXVzeHRiIiwicm9sZSI6ImFub24iLCJpYXQiOjE2OTgwODYyNTUsImV4cCI6MjAxMzY2MjI1NX0.I3v1fYevo3rzWOT8KvkIVDrZ0LbyvABN6YaynXIYE4I");

    const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm<CircuitoFormValues>({
    ...
    return (
        <div className="px-24">
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <Cabecera titulo={isEditing ? 'Editar circuito' : 'Crear circuito'} subtitulo={isEditing ? 'Edita los datos del circuito' : 'Rellena los datos del circuito'} />
                {loading ? <Loading /> :
                    <div className="mt-5 bg-gray-50 p-5 rounded-lg">
                        <div className="w-96 flex flex-col gap-4">
                            <div>
                                <div className="w-full px-3">
                                    ...
                                    {
                                        errors.trazado && (
                                            <span className="text-red-500 text-xs italic">{errors.trazado.message as string}</span>
                                        )
                                    }
                                    ...
                                    file:bg-gray-200 file:hover:bg-gray-300 hover:bg-gray-200"
                                        id="titulo" type="file" placeholder="Imagen" {...register("trazado", {
                                            // required: {
                                    ...
                                    />
                                    {
                                        isEditing && (
                                            <div>
                                                <p className="text-gray-600 text-xs italic">Imagen del circuito</p>

                                                <img className="w-48 rounded-lg border-2 mb-4" src={Constantes.IMAGE_BASE_URL + editingTrazado
                                                } alt="" />
                                            </div>
                                        )
                                    }
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <InputSelectField label="País"
                                    register={register} name="pais"
                        ...
                        <div className="flex justify-start mt-5">
                            <InputButton label={loading ?
                                isEditing ? 'Guardando...' : 'Creando...'
                                : isEditing ? 'Guardar cambios' : 'Crear circuito'} loading={loading} />
