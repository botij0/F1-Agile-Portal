...
import VotacionRow from "@/app/components/Votaciones/VotacionRow";
...
const VotacionesMng = () => {
    ...
    const [votaciones, setVotaciones] = useState<any[] | null>(null);
    ...
    const paginateVotaciones = paginate(votaciones, currentPage, pageSize);
    ...
    const deleteVotacion = (e: React.MouseEvent, id: number) => {
        e.preventDefault();
        deleteRequest("votaciones/" + id).then((res) => {
            if (votaciones) {
                setVotaciones((prevElement) => {
    ...
    return (
        <div className="container mx-auto my-8">
            ...
            <h2 className="text-black text-2xl">Gestión de Votaciones</h2>
                ...
                <table className="min-w-full">
                        ...
                        loading && (
                            <tbody>
                                <tr>
                                    <td></td>
                                    <td></td>
                                    <tr role="status" className="p-5 w-max">
                                        <svg
                                            aria-hidden="true"
                    ...
                    {!loading && (
                        <tbody className="bg-gray-50">
                            {paginateVotaciones.map((votacion) => (
                                <VotacionRow
                                    key={votacion.id}
                                    votacion={votacion}
                                    deleteVotacion={deleteVotacion}
                                />
                            ))}
                        </tbody>
                    )}
            ...
            <Pagination
                items={votaciones == null ? 0 : votaciones.length}
                pageSize={pageSize}
