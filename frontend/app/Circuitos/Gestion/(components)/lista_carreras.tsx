...
const ListaCarreras = (
    { carreras, getCarreras }: { carreras: Carreras[], getCarreras: Function }
) => {

    function handleDelete(id: any) {
        if (confirm('¿Estás seguro de que quieres eliminar esta carrera?')) {
            deleteRequest('carreras/' + id)
                .then(response => {
        ...
        carreras.length > 0 && (
                        ...
                        <tbody>
                            {
                                carreras.map((carrera, index) => (
                                    <tr key={index} className="text-gray-900 bg-gray-100 border-b border-gray-100 hover:bg-white">
                                        <td className=" px-4 py-2 text-center">{index + 1}</td>
                                        <td className=" px-4 py-2 text-center">{carrera.fecha}</td>
                                        <td className=" px-4 py-2 flex items-center justify-center">
                                            <button className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg mr-2" onClick={() => { handleDelete(carrera.id) }}>
                                                Eliminar
                                            </button>
                                        </td>
                                    </tr>
            ...
            <div className="mt-5 bg-gray-50 p-5 rounded-lg">
                ...
                <div className="flex flex-wrap -mx-3 mb-6 bg-gray-50 rounded-lg p-5">
                    <h2 className="text-lg font-bold"> No hay carreras para mostrar </h2>
