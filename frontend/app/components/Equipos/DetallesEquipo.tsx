...
function DetallesEquipo(params: { id?: string }): React.JSX.Element {
  ...
  if (id && !requested) {
    setRequested(true);
    getRequest("equipos/" + id)
      .then((response) => {
        if (response.data.success) {
          const { logo, nombre, twitter, circuitos, pilotos, coches } =
            response.data.data;
          setLogo(logo);
          setNombre(nombre);
          setTwitter(twitter);
    ...
    <div className="mt-[10px]">
      <div className="2xl:flex mt-[30px] max-w-[90%] mx-auto">
        <div className="mx-auto 2xl:me-5">
          <h1 className="font-bold text-center text-3xl text-black mb-5 underline">
            {varNombre}
          </h1>
        </div>

        <img
          src={Constantes.IMAGE_BASE_URL + varLogo}
          alt={"Cover"}
          width={100}
          height={100}
          className="mx-auto"
      ...
      <div className="max-w-[900px] mx-auto mt-5">
        <h3 className="font-bold text-center text-2xl text-black mb-5 underline">
          Circuitos
        </h3>
